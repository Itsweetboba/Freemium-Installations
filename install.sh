...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENSE CHECK                     [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "Seraphine" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy First Please, You Got No Access${NC}"
    echo -e "${YELLOW}TELEGRAM : @PaiiTheStory${NC}"
    echo -e "${YELLOW}WHATSAPP : 6283854772553${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 15K Free Update If Token Changed${NC}"
    echo -e "${YELLOW}©PaiiTheStory${NC}"
    exit 1
  fi
  clear
}
...
read -p "Enter the Configure token to run wings: " wings
...
hackback_panel() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]                    HACKBACK PANEL                 [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  # Minta input dari pengguna
read -p "Input Username Panel : " user
read -p "Input Password : " psswdhb
  #!/bin/bash
cd /var/www/pterodactyl || { echo "Direktori tidak ditemukan"; exit 1; }
...
while true; do
  ...
  echo -e "${BLUE}'ggg      ,gg'''  .    ggg       Auto Installer PaiiTheStory Private   ${NC}"
  echo -e "${BLUE}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${BLUE}ggg:     gg.     -   ,ggg       • Telegram : t.me/PaiiTheStory     ${NC}"
  echo -e "${BLUE} ggg:     ggg._    _,ggg        • Credit  : PaiiTheStory ${NC}"
  echo -e "${BLUE} ggg.    '.'''ggggggp           • Support by PaiiTheStory  ${NC}"
